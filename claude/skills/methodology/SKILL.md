...
| **T4 — Post-merge CI** | After merge, where CI exists | The merged commit's own check-runs, classified against the pre-merge commit | polled, async |
...
- **T3 and T4 are where faults are actually found — do not skip them to save time.** Audited against this repo's own history (14 post-implementation faults over ~6 months, commits `51c57bd`, `331d3af`, `31a6d52`, `c19fe01`): 12 were found by review reading the diff, 2 by post-merge CI, and **0 by a test suite run at any breadth**. The faults were injection vectors, a fail-open safety gate, stale cross-invocation state, error-path ordering, and cross-task doc drift — none of which a test written for the task's happy path catches, regardless of how wide it runs. T1 is where the *time* is saved; T3/T4 are where the *quality* is preserved. Trading T3 or T4 away for speed is not the same trade as narrowing T1, and is not authorized by this section.
- **A clean local verify is not a CI pass.** One audited fault (`31a6d52`) was a task's own regression test passing locally and failing on main's post-merge CI. Local rigor of any tier cannot catch an environment difference — that is what T4 is for.
...
- **Where the suite is thin, T1 and T2 are the same empty check.** `bin/bugfix-worker` carried 9 of the 14 audited faults and has no test file; `make test` runs two unrelated pytest files. Before budgeting tiers, confirm the tier you are relying on actually covers the changed code — if it does not, the detection load falls entirely on T3, and the review is not optional.
