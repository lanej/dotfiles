...
- **`easypost-platform.engineering_demand.classified_tickets`** — Engineering tickets from Phabricator + Jira + ADO, AI-classified by category (`vendor_driven`, `carrier_mandated`, `maintenance`, `support_ops`, etc.) and tagged with `carrier_family`. **`cycle_days` is calendar lead time (ticket open→resolved), NOT engineering effort.** Using `SUM(cycle_days) × daily_rate` inflates cost estimates by 10–100×. Correct formula for cost: `ticket_count × bd_per_ticket_calibrated × daily_rate` where `bd_per_ticket` comes from a ground-truth source (e.g. Phabricator task data). `carrier_family` is an AI-generated label that changes when the classifier is retrained — do not treat as a stable identifier over time. USPS carrier_family split (as of 2026-05-23): `PCP` = PC Postage accounts (canonical USPS), `Ship` = Ship API accounts (canonical USPSShip). Confidence threshold: filter to `>= 0.55`.
