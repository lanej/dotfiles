...
- **epq cache write_cache() signature**: Only accepts `records` (list[dict]) and `scalars` (dict) kwargs — no arbitrary keyword arguments. Multiple data series require separate `write_cache()` calls with distinct cache keys (e.g., `write_cache("db_historical", ...)`, `write_cache("db_monthly", ...)`), not extra kwargs on one call.
- **epq dark mode color constants**: `epq.dark` only exports `BAR_BLUE`, `BG`, `EDGE`, `RCPARAMS`, `TEXT`. There are no `dark.TEAL`, `dark.CORAL`, `dark.GOLD`, or `dark.NAVY` — using them raises `AttributeError` at render time. Semantic accent colors (CORAL, GOLD) are used directly from `style.*` in both light and dark modes; only bar/background/edge colors swap to `dark.*` variants.
- **epq jira: frontmatter on first render**: The post-render hook (`restore_frontmatter`) only injects frontmatter into the rendered `.md` when a `gspace:` block is present in the QMD or prior saved state exists. On the **first render** of a new QMD with a `jira:` block but no `gspace:` block, the `.md` will have no frontmatter — `jira issues sync` will fail. Fix: manually prepend the `jira:` YAML block to the `.md` file after the first render, then sync. Subsequent renders will preserve it via the save/restore cycle. (Same issue as EP-36 obs 4587.)
- **jira issues sync conflict after API operations**: Any Jira API call that touches a ticket (link create/delete, `jira_issues_update`) updates its `updated` timestamp. The next `jira issues sync` will detect a conflict and fail. Resolution: run `jira issues pull <KEY> <file.md>` first to refresh the local timestamp, then re-render and sync.
- **"Link to a Jira ticket" in QMD prose means inline markdown hyperlink**, not a Jira issue link edge. Use `[EP-XX](https://simplerpostage.atlassian.net/browse/EP-XX)` in the prose. Creating a structural dependency link (Blocks, Relates, etc.) is a separate Jira operation and requires explicit instruction.
