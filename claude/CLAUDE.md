...
- **matplotlib + LaTeX (Quarto PDF)**: Dollar sign escaping is context-dependent — the rule differs between QMD prose/fmt strings and matplotlib labels. **In QMD LaTeX prose and `epq fmt` strings**: `$` gets consumed by lualatex — escape with `\\$` (e.g., `f"\\${val:.1f}M"`). **In matplotlib f-strings** (axis labels, `ax.text()`, bar labels, formatters): use plain `$` — matplotlib is NOT LaTeX; `\\$` produces a literal backslash before the dollar sign in the rendered label. `fmt.millions_formatter()` expects raw dollar values — NEVER pre-divide data by 1e6 before applying this formatter; doing so produces `$0.0M` for all axis ticks. Plot raw values and let the formatter scale. **Unicode math in QMD prose**: the EPQ audit rule `prose/unicode-math` FAILs the render for ALL unicode math operators — `×`, `≥`, `≤`, `≠`, `≈`, `→`, `↑`, `↓` — no exceptions regardless of font. Use plain English only: "times", "at least", "at most", "x". `ax.text()` does not accept `set_clip_on` as a kwarg — capture the returned Text object and call the method: `t = ax.text(...); t.set_clip_on(True)`. **Unicode arrows `↑↓` (U+2191/U+2193) and `→` (U+2192) do not render in matplotlib via Helvetica Neue or lualatex** — use ASCII `+`/`-`/`>` instead. **`matplotlib.cycler` removed in mpl 3.10** — `from matplotlib.cycler import cycler` raises `ModuleNotFoundError`; use `from cycler import cycler` (standalone package, already in matplotlib's dependency tree).
...
- **QMD inline `{python}` prose — numpy scalar types**: pandas/numpy aggregation results (`df["col"].sum()`, `.count()`, etc.) return `numpy.int64` / `numpy.float64` which render as `np.int64(16)` or `np.float64(2.0)` in the PDF, not `16` or `2.0`. Always convert before embedding in prose: `int(...)` for counts, `float(...)` for ratios, or `f"{value:.1f}"` for formatted strings. Never embed a raw pandas/numpy scalar directly in a `{python}` inline expression.
