...
- **epq `figure/no-style-import` detection**: The rule matches the literal substring `"from epq import style"`. A combined import like `from epq import cache, style, fmt` does NOT match — `cache` appearing before `style` breaks the substring. Always import cache on a separate line: `from epq import cache` / `from epq import style, fmt`.
