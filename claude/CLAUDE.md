...
- **Chrome headless PDF rendering**: Use `--no-pdf-header-footer` (not `--print-to-pdf-no-header`) to suppress date/URL/page-number chrome. Use `--no-margins` to honor `@page` margin CSS. To scale HTML content to fill a letter page, add `zoom: 1.1` to `body` CSS and tune until content fills ~95% of page without overflow. Full command: `"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless=new --disable-gpu --print-to-pdf=out.pdf --no-pdf-header-footer --no-margins "file:///abs/path/to/file.html"`
