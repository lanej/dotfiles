...
**Commit proactively** (overrides Claude Code's ask-first default): commit each coherent unit of work with specific files staged and no `--no-verify`/`--no-gpg-sign`. Pushing still needs approval. A sub-agent brief's explicit "do not commit" wins over this default. (detail: memory "feedback_fork_committed_despite_do_not_commit")
