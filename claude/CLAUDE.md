...
**SQL field verification** — Before writing a BQ query against any table not explicitly described via `describe_table` in the current session, run `mcp__bigquery__describe_table` first. "Familiar" is not a valid exception — tables assumed to share a schema with another (e.g. `phab_tasks_raw` assumed to match `classified_tickets_raw`) routinely differ. When a query fails with a column-not-found error, use `describe_table` immediately; do not guess alternate column names. **This rule applies equally during spec and planning phases** — never assume a column exists in a spec or design doc without verifying first; deferring verification to "implementation time" embeds false assumptions into written requirements. If a spec references a specific column name, run `describe_table` before writing the spec line. Failure mode: spec assumed `CONVERSATION_START` on `gong_landing.CALL_TRANSCRIPTS`; column didn't exist; discovered at implementation with a fallback required. **Join propagation**: a field existing on a base table does not mean it propagates through a JOIN — verify by running a sample query with the actual join in place before building on it. Failure mode: `unified_identity.team_display_name` exists on the base table but returns NULL through a `task_event_durations LEFT JOIN unified_identity` because the column is computed in a view layer that doesn't expose it through that join path.
