...
vim.api.nvim_set_keymap('n', '<leader>cf', '<cmd>lua require("fzf-lua").fzf_exec("git diff $(git merge-base --fork-point origin/master) --name-only", { preview = "echo {} | xargs -n 1 -I {} git diff $(git merge-base --independent origin/master) --shortstat --no-prefix -U25 -- {} | delta", actions = require("fzf-lua").defaults.actions.files })<CR>', {
  noremap = true,
  silent = true,
})
